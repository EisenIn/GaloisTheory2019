...
f_1(x) & = & x^{4} - x^{3} - x^{2} + 6
 ...
 \emph{Hint: Check the sage code that I wrote to latex the example-run of the Euclidean algorithm and modify it. The code can be found in the {\tt Programming} directory of the GitHub page of our course.} 
\item Which of the following polynomials in   $\mathbb{Q}[x]$ are separable?  
  \begin{eqnarray*}
    f_1(x) & = & x^4 + 3x^2 + 1) (x^{20} + 3x^{15} + 3  \\
...
\item Let $R$ be an integral domain. Show that $R[X]$ is an integral domain. [Hint: If $f,g\in R[X]$ show that $deg(fg)=deg(f).deg(g)$] 
